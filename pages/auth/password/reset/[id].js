...
const ResetPassword = ({ router }) => {
    ...
    useEffect(() => {
        ...
        if (decoded) setState({ ...state, name: decoded.name, token: router.query.id }); //We will send this token to backend
    }, [router]);
    ...
    const passwordResetForm = () => (
        <form onSubmit={handleSubmit}>
            ...
            <div>
                <button className="btn btn-outline-success">{buttonText}</button>
            </div>
        </form>
    );
